<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:key name="author-key" match="/feedback/reviews/review" use="@author" />
  <xsl:template match="/feedback">
	...
	<xsl:for-each select="reviews/review[generate-id(.)=generate-id(key('author-key', @author)[1])]">
	  ...
	  <xsl:for-each select="key('author-key', @author)">
		...
		<w:pStyle w:val="Heading2"/>
	      ...
	      <w:r>
		<w:t>Feedback from <xsl:value-of select="@reviewer"/>
		</w:t>
	      </w:r>
	    </w:p>
	    <w:p>
